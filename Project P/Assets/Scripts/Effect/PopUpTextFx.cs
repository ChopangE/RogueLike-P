...
public class PopUpTextFx : MonoBehaviour
{
    ...
    [SerializeField] private float disapeearingSpeed;
    ...
    private void Start()
    {
        myText = this.GetComponent<TextMeshPro>();
        textTimer = lifeTime; 
    }
    ...
    private void Update()
    {
        ...
        if(textTimer < 0)
        {
            float alpha = myText.color.a - colorDisappearingSpeed;
            myText.color = new Color(myText.color.r, myText.color.g, myText.color.b, alpha);

            if (myText.color.a < 50)
                speed = disapeearingSpeed;
